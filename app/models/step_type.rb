...
class StepType < ActiveRecord::Base
  ...
  def condition_groups_init
    cgroups = condition_groups.each_with_object({}) do |condition_group, memo|
    ...
    agroups = actions.include_subject_condition_groups.each_with_object(cgroups) do |action, memo|
      ...
      memo[name][:facts].push(cssClasses: fact_css_classes[action.action_type],
                              name: name,
  ...
  def actions_for_condition_group(condition_group); end

  def actions_for(assets)
    # condition_group_classification_for(assets)
  end

  def explain(assets); end
